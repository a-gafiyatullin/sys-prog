...
PicoHTTPParser is a tiny, primitive, fast HTTP http_header/response parser.
...
The example below reads an HTTP http_header from socket `sock` using `read(2)`, parses it using `phr_parse_request`, and prints the details.
...
while (1) {
    ...
    prevbuflen = buflen;
    buflen += rret;
    /* parse the http_header */
    num_headers = sizeof(headers) / sizeof(headers[0]);
    pret = phr_parse_request(buf, buflen, &method, &method_len, &path, &path_len,
                             &minor_version, headers, &num_headers, prevbuflen);
    if (pret > 0)
        break; /* successfully parsed the http_header */
    else if (pret == -1)
        return ParseError;
    /* http_header is incomplete, continue the loop */
    assert(pret == -2);
    if (buflen == sizeof(buf))
        return RequestIsTooLongError;
}

printf("http_header is %d bytes long\n", pret);
